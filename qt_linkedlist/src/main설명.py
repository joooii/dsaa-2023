...
class DoublyLinkedList:
    ...
    def remove(self, data):
        if data.prev_node:
            data.prev_node.next_node = data.next_node
        if data.next_node:
            data.next_node.prev_node = data.prev_node
        if data == self.head:
            self.head = data.next_node
        if data == self.tail:
            self.tail = data.prev_node
        return data.next_node or data.prev_node
    ...
    def merge(self, other_list: 'DoublyLinkedList'):
        ...
        if self.head is None:  # 만약 현재 리스트가 비어 있다면
            self.head = other_list.head
            self.tail = other_list.tail
            return
        
        self.tail.next_node = other_list.head  # 현재 리스트의 마지막 노드와 다른 리스트의 첫 번째 노드를 연결
        other_list.head.prev_node = self.tail  # 다른 리스트의 첫 번째 노드의 이전 노드를 현재 리스트의 마지막 노드로 설정
        self.tail = other_list.tail  # 현재 리스트의 마지막 노드 업데이트

        # 원형 연결 리스트의 처음과 마지막 노드를 연결
...
class ImageSlider(QMainWindow):
    ...
    def __init__(self):
        ...
        self.current_node = None    # 현재 보여주고 있는 이미지 추적
        self.current_list = self.dllist1    # 현재 작업중인 연결 리스트를 일단 dllist1로 설정 -> 병합 이후 current_list는 dllist로 변경됨

    def initUI(self):
        self.setGeometry(400,400,500,600)
        self.setWindowTitle('Image Slide Show')
        central_widget = QWidget(self)  # 중심 위젯 생성
        self.setCentralWidget(central_widget)  # QMainWindow의 중심 위젯 설정
...
## 함수 ##
    ...
    def folder_select(self):
        folder_dir = 'C:/Users/김주희/OneDrive - 부경대학교/workspace/qt_linkedlist/resources'
    ...
    # 이미지 추가
    def add_image(self):  
        image_dir = 'C:/Users/김주희/OneDrive - 부경대학교/workspace/qt_linkedlist/resources'
    ...
    def delete_image(self):
        if self.current_node:
            ...
            if self.current_node:  # 아직 이미지가 남아있다면
    ...
    def merge_linked_list(self):
        ...
        if self.dllist.head: # 병합된 이미지가 있을 때
